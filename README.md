...
- 🧩 **脚本拆分** - `jd_cookie_sync.js` 负责 Cookie，`jd_ws_sync.js` 负责 WSKEY
- 🔄 **智能同步** - 自动同步到青龙面板环境变量
...
打开京东 APP，随意浏览商品或进入"我的"页面，`jd_cookie_sync.js` 会自动抓取 Cookie 并同步到青龙面板的 `JD_COOKIE`。

WSKEY 会由 `jd_ws_sync.js` 在京东相关请求中自动抓取并同步到青龙面板的 `JD_WSCK`。如果 pin 和 wskey 分别出现在相邻请求里，脚本会用短时缓存配对，避免串号。
